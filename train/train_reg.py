...
def run_epoch(loader, model: nn.Module, cfg: Config, optimizer=None, training: bool = False):
    ...
    for x1, x2, y in pbar:
        x1, x2, y = prep_batch(x1, x2, y, cfg.device)
        ...
        with torch.set_grad_enabled(training):
            ...
            if isinstance(out, (tuple, list)):
                ...
                reg_tuple = out[3] if len(out) > 3 else None
            else:
                main, aux2, aux3, reg_tuple = out, None, None, None, None
            ...
            λ = 1e-3  
            if reg_tuple is not None and len(reg_tuple) > 0:
                reg = λ * sum(reg_tuple)
            else:
                reg = 0.0
            loss = loss_main + 0.6 * loss_aux2 + 0.4 * loss_aux3 + reg
...
def train(train_dataset, val_dataset, model: nn.Module, cfg: Config):
